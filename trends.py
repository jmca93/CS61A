...
def tweet_location(tweet):
    """Return a position representing a tweet's location."""    #from geo.py
...
def find_state_center(polygons):
    ...
    """
    cx, cy, a, i = 0, 0, 0, 0
    centroid_list = list(map(find_centroid, polygons))
    while i < len(centroid_list):
        a += centroid_list[i][2]
        cx += (centroid_list[i][0] * centroid_list[i][2])
        cy += (centroid_list[i][1] * centroid_list[i][2])
        i += 1
    if a == 0:
        return 'Error'
    else:
...
def average_sentiments(tweets_by_state):
    ...
    state_names = list(tweets_by_state.keys())
    name = 0
    for name in state_names:
        i = 0
        sentiment_count = 0
        total_sent = 0
        while i < len(tweets_by_state[name]):
            if has_sentiment(analyze_tweet_sentiment(tweets_by_state[name][i])):        #analyze_tweet_sentiment(tweets_by_state[name][i]) != 0
                total_sent += sentiment_value(analyze_tweet_sentiment(tweets_by_state[name][i]))
                sentiment_count += 1
            i += 1
        if sentiment_count != 0:
            averaged_state_sentiments[name] = total_sent / sentiment_count
